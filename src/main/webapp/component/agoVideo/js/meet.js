(function($) {
    ...
    $(function() {
        var tmpPath = getParamsValue("tmpPath");
        var tmpAppId = getParamsValue("appId");
        var tmpChannel = getParamsValue("channel");
        console.log(tmpAppId+ ", "+ tmpChannel);

        var resolution = "480p",
            maxFrameRate = 15,
            channel = tmpChannel, // 使用进件号
            key = tmpAppId, // appId
			uid = 666, // 666代表客服
            cId=999, //999代表客户
            remoteStreamList = [],
			...
			recordingServiceUrl =  tmpPath + '/lendSign',
            recording = false,
            localStream,
        ...
        (function initAgoraRTC() {
            client.init(key, function(obj) {
				...
				$.get(recordingServiceUrl + "/channelKey/"+channel+"/"+uid + "/generate")
                     .done(function(data) {
					console.log(data);
					client.join(data, channel, uid, function(uid) {
        ...
        (function(global) {
            ...
            // that's it! modify by yhl Base64 ==> _Base64
            if (global['Meteor']) {
                _Base64 = global.Base64; // for normal export in Meteor.js
            }
        })(window);
        ...
        function subscribeStreamEvents() {
            client.on('stream-added', function(evt) {
                ...
                if(streamId === cId){
                    ...
                    $.get(recordingServiceUrl + "/recordingKey/"+channel+"/"+uid + "/generate")
                        .done(function(data) {
                            console.log(data);
        ...
        function subscribeMouseClickEvents() {
            ...
            $(".end-call-button,.list-hang-up-btn").click(function(e) {
                client.leave();
                window.close();
                //window.location.href = "ago1.html";
            });
